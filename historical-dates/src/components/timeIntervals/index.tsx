...
import { ArrowControlsStatus } from "../ArrowControls/ArrowControls";
...
function TimeIntervals({
  ...
  const ancestorRef = React.useRef<HTMLDivElement | null>(null);
  const activePointNumberRef = React.useRef<HTMLSpanElement | null>(null);
  const activePointRef = React.useRef<HTMLDivElement | null>(null);
  const prevPointNumberRef = React.useRef<HTMLSpanElement | null>(null);
  const prevPointRef = React.useRef<HTMLDivElement | null>(null);
  ...
  React.useEffect(() => {
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      // если есть активная точка, то прячем ее
      ...
      <div className="time-intervals" ref={ancestorRef}>
        ...
        {pointsData.map(({ id, index, label }) => (
          <Point
            ...
            index={index}
            label={label}
            pointClickHandler={() => handleBulletClick(index)}
          />
        ))}
      </div>
    </>
